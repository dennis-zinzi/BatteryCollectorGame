...
void ABatteryCollectorCharacter::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	...
	//Setup keypress to start collecting action
...
void ABatteryCollectorCharacter::CollectPickups(){
	//Get all overlapping Actors, then store them in an array
	...
	for(int32 iCollected = 0; iCollected < collectedActors.Num(); ++iCollected){
		...
		//If cast successful, and pickup valid and active
		if(testPickup && !testPickup->IsPendingKill() && testPickup->IsActive()){
			//Call the pickup's WasCollected() func
...
//Called whenever power is increased/decreased
